...
| 구독자 일괄 발송(`RecruitmentManagementService.sendToAllSubscribers()`) 병렬화 | 지금은 구독자 전원에게 `forEach`로 순차 발송한다(1인당 SMTP 왕복 + `email_log` 쓰기). "SQLite `HikariCP maximum-pool-size: 1`이라 병렬화해도 어차피 DB 쓰기에서 줄 서니 의미 없다"는 가설을 세웠다가 재검토해서 기각했다 — **DB 쓰기(로컬, 수 ms)만 병렬화해도 못 줄이는 것이지, SMTP 왕복(외부 네트워크, 최대 5초)은 DB 커넥션과 무관해 병렬로 겹칠 수 있다.** 예: 구독자 100명·SMTP 1초·DB쓰기 5ms 가정 시 순차 ≈100초, "SMTP만 병렬 + DB쓰기만 줄서기" ≈1.5초. OCI Email Delivery 쪽 발송 속도 제한도 병목이 아니다 — 이 프로젝트 계정은 PAYG(`infra/CLAUDE.md`)라 분당 18,000통까지 허용되고(Always Free/Trial만 분당 10통으로 낮음), [OCI 공식 Limits by Service](https://docs.oracle.com/en-us/iaas/Content/General/service-limits/default.htm)의 `sendrate` 한도 표로 확인됨. 즉 병렬화를 막는 실제 기술적 제약은 없고, 지금 순차인 건 그냥 제일 단순한 구현이라 그런 것으로 보인다(코드에 "왜 순차로 짰는지" 근거 주석 없음). 실제로 바꿀지는 지금 규모(수십~수백 명)에서 100초 안팎 지연이 문제가 되는지에 달려있다 — 안 되면 굳이 복잡도(스레드 관리, 부분 실패 처리)를 더할 이유 없음. |
