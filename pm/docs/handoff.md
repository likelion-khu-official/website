...
> 2026-07-27 정리: 이 역할의 마인드셋·지표·필요 역량·평소 루틴·협업 인터페이스·계정 인벤토리를 여기로 모았다(예전엔 `infra/RUNBOOK.md`에 있었는데, RUNBOOK은 "문제 맞닥뜨렸을 때 뭘 하나"만 담고 "이 역할이 뭐고 어떻게 하는가·다음 사람에게 뭘 넘기는가"는 여기로 분리했다).
...
| 수동 배포·롤백 | [`infra/RUNBOOK.md`](../../infra/RUNBOOK.md)의 "자주 쓰는 명령" — **태그를 반드시 명시**해야 한다(미지정 시 위험, learnings 참고) |
...
| 항목 | 내용 |
|---|---|
| **배포가 언제 도는가** | `backend/**`·`shared/**` 변경이 있는 PR이 `dev`/`main`에 머지될 때만 CD가 자동으로 돈다. `infra/**`만 바뀐 커밋은 안 돈다([`infra/RUNBOOK.md`](../../infra/RUNBOOK.md)의 "자주 쓰는 명령" 참고). |
...
**받으면 뭘 해야 하는지**는 이 문서 초안 이후 채워졌다 — [`infra/RUNBOOK.md`](../../infra/RUNBOOK.md)에 알람별(디스크·메모리·백업부재·git드리프트·UptimeRobot DOWN) 확인·조치·에스컬레이션 절차를 정리했다. 인프라 담당자 교체 시 이 문서부터 넘기면 된다.
...
- [x] 알람별 대응 절차(runbook)를 문서화한다 — [`infra/RUNBOOK.md`](../../infra/RUNBOOK.md) 완료.
...
- 최고관리자 계정 승계는 이미 별도 기능(Story [#203](https://github.com/likelion-khu-official/website/issues/203) "최고관리자가 다음 대표에게 자리를 넘긴다")으로 진행 중 — 이 문서가 다루는 "우리끼리 손 떼기"와는 다른 층위(제품 기능 vs 팀 운영)라 겹치지 않는다.
