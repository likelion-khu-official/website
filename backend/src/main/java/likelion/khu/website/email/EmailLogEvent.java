...
record EmailLogEvent(String recipient, EmailType emailType, String subject, EmailStatus status,
    ...
    static EmailLogEvent success(String recipient, EmailType emailType, String subject, String messageId) {
        return new EmailLogEvent(recipient, emailType, subject, EmailStatus.SUCCESS, null, null, messageId);
    }

    static EmailLogEvent failure(String recipient, EmailType emailType, String subject,
                                  String errorMessage, FailureCause failureCause, String messageId) {
        return new EmailLogEvent(recipient, emailType, subject, EmailStatus.FAILURE, errorMessage, failureCause, messageId);
    }

    EmailLog toEmailLog() {
        return status == EmailStatus.SUCCESS
                ? EmailLog.success(recipient, emailType, subject, messageId)
                : EmailLog.failure(recipient, emailType, subject, errorMessage, failureCause, messageId);
    }
}
