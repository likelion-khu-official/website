...
2. **마이그레이션 파일 작성 — 버전은 타임스탬프로**
   ```bash
   date +V%Y%m%d%H%M%S
   ```
   위 값에 `__설명.sql`을 붙여 신규 생성(예: `V20260728143000__application_form_and_applications.sql`). Flyway는 버전을 숫자로만 비교하므로 기존 `V1~V7`(순번) 옆에 타임스탬프 버전이 그대로 이어 붙는다 — 기존 파일을 리네임할 필요 없음. **머지된 파일은 절대 수정하지 않는다**(체크섬 불일치 시 stage는 자동 초기화, prod는 기동 실패).

   **왜 순번(`V{n+1}`)이 아니라 타임스탬프인가:** 순번은 "지금 dev에 뭐가 있는지" 봐야 다음 번호를 알 수 있는데, 다른 사람이 동시에 같은 걸 보고 있으면 둘 다 같은 번호를 잡는다 — 각 PR 단독으로는 통과하다가 dev에 합쳐지는 순간에만 드러난다(실제로 서로 다른 두 PR이 각자 `V7`을 만들었고, 둘 다 dev에 먼저 머지된 `V7__add_staff_activities.sql`과 충돌해 관련 없어 보이는 마이그레이션 테스트가 무더기로 깨졌다 — Flyway가 버전 중복을 감지하면 `FlywayException`을 던지고, 그게 Flyway를 쓰는 모든 테스트의 Spring 컨텍스트 기동 자체를 막기 때문). 타임스탬프는 각자 로컬 시계만 보면 되니 이 조율 자체가 필요 없어진다. 그래도 혹시 겹치면(같은 초에 커밋 등) `migration-guard` CI가 머지 시점에 버전 번호 중복을 기계적으로 잡아준다(2026-07-28 추가, 아래 10번).
...
10. **`migration-guard` CI 잡이 두 가지를 기계적으로 강제한다(`.github/workflows/ci.yml`).**
    - **버전 번호 중복 확인(2026-07-28 추가).** `actions/checkout`이 PR merge ref를 받으므로 이 시점 마이그레이션 디렉터리엔 머지 대상 브랜치 + 이 PR 파일이 전부 있다 — 같은 버전 번호(`V` 다음, `__` 앞)를 쓰는 파일이 둘 이상이면 그 자리에서 CI가 실패한다. 위 2번에서 타임스탬프 버전을 권장하는 이유가 바로 이 사고를 애초에 안 겪게 하는 것이고, 이 가드는 그래도 겹쳤을 때의 마지막 안전망이다.
    - **재생성·행 삭제 패턴에 업그레이드 테스트 누락 확인.** 새로 추가된 마이그레이션 파일에 `DROP TABLE`이나 `DELETE FROM`이 있는데 같은 PR에 `backend/src/test/.../migration/` 아래 테스트 변경이 없으면 CI가 그 자리에서 실패한다. 시나리오표 기준 컬럼 추가(nullable)·컬럼 이름 변경(RENAME COLUMN) 두 경우만 이 두 키워드 없이 처리되고, 나머지(컬럼 삭제·행 삭제·테이블 삭제·CHECK 신규/좁히기·FK 추가)는 전부 재생성 패턴을 쓰므로 이 가드에 걸린다. **이건 "테스트를 아예 안 쓴 것"만 잡는다 — 테스트를 쓰긴 했는데 내용이 V6처럼 불완전한 경우(값 일부만 시드)는 여전히 리뷰어·본인이 확인해야 한다.**
...
11. **PR 본문에 체크 블록 포함**
   ...
   - [x] (행을 삭제하는 마이그레이션이면) stage/prod 배포 직전 `infra/scripts/backup-db.sh` 수동 실행 — 정기 백업(1일 1회)만으론 최대 24시간 데이터가 무방비
