...
**이 패턴을 항상 지켜야 하는 이유가 하나 더 늘었다(2026-07-31, #320):** `cd.yml`이 이번 배포에 새로 추가된 마이그레이션 파일을 이 패턴(`drop table`+`rename to`)으로 자동 판별해서, 컬럼 삭제·이름변경형이면 배포 실패 시 자동 롤백을 건너뛰고 사람에게 넘긴다(구버전 앱이 사라진 컬럼을 찾다 또 깨질 수 있어서 — `infra/CLAUDE.md` "수동 배포·롤백" 참고). 이 판별은 SQL 텍스트를 grep하는 휴리스틱이라, **이 패턴을 벗어난 방식으로 컬럼을 지우거나 바꾸면 CD가 "추가형(안전)"으로 잘못 판단해서 위험한 자동 롤백을 그대로 실행할 수 있다.** 컬럼 삭제·변경이 필요하면 항상 이 문서의 재생성 패턴 그대로 쓸 것.

**같은 판별이 `UPDATE`/`DELETE` DML도 본다(2026-07-31 실측 보강).** 기존 행을 바꾸거나 지우는 마이그레이션(백필·정정 스크립트 등)은 컬럼 삭제·이름변경이 아니라도 자동 롤백 전에 사람이 보게 한다 — 롤백은 앱 이미지만 되돌릴 뿐 이미 실행된 `UPDATE`/`DELETE`는 그대로 남기 때문이다. `INSERT`(행 추가)는 계속 안전 취급이니 시드 데이터 삽입 같은 건 예전처럼 자유롭게 써도 된다 — 단, 시드 데이터를 멱등하게 넣고 싶으면 `INSERT OR IGNORE`를 쓸 것. **`INSERT OR REPLACE`/`REPLACE INTO`는 쓰지 말 것** — 이름은 INSERT지만 PK/UNIQUE 충돌 시 기존 행을 삭제하고 새로 넣는 사실상 DELETE+INSERT라(다른 컬럼값이 조용히 사라질 수 있음) 같은 위험으로 판정된다.
