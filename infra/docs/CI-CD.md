...
`migration-check` job과 아래 "실측 검증 이력"을 이해하려면 Flyway가 정확히 뭘 하는지 알아야 한다. 세부 규칙(SQLite 컬럼 변경 시 테이블 재생성 패턴 등)은 [`db-migration.md`](./db-migration.md)가 정본 — 여기는 CD가 왜 이렇게 설계됐는지 이해하는 데 필요한 개념만 다룬다.

### 기본 동작 — 버전 파일을 순서대로, 딱 한 번만

`backend/src/main/resources/db/migration/`에 `V{버전}__설명.sql` 형태로 SQL 파일을 쌓아두면, 앱이 뜰 때마다 Flyway가:
1. `flyway_schema_history`라는 **장부 테이블**을 보고 "이미 적용된 버전"을 확인한다.
2. 장부에 없는(=아직 안 적용된) 파일만 버전 순서대로 실행한다.
3. 실행한 파일은 장부에 "적용됨 + 그때 파일의 체크섬"으로 기록한다.

즉 "지금 DB 스키마가 어떤 모습이어야 하는가"는 이 SQL 파일 목록 자체가 정의고, 장부는 "그중 어디까지 실제로 실행했는지"의 기록이다. 로컬·stage·prod가 같은 파일 목록을 같은 순서로 실행하면 결국 같은 스키마가 된다는 게 Flyway를 쓰는 이유다.

### `validate()` — 장부와 파일이 지금도 같은지 대조

`migrate()`를 실행하기 전에 Flyway는 항상 장부와 파일을 대조한다(`validate`). 여기서 걸리는 불일치는 두 가지로, **원인이 다르다**:

- **파일이 없어짐** — 장부엔 "V190000 적용됨"인데 그 파일이 지금 레포에 없음 (예: `git rm`으로 정리)
- **체크섬이 다름** — 파일은 있는데 내용이 바뀜 (예: 이미 배포된 파일을 누가 수정)

둘 다 같은 예외(`FlywayValidateException`)로 던져지지만, 이 프로젝트는 원인별로 다르게 잡는다 — `cd.yml`의 `migration-check`가 `git ls-tree`로 삭제(REMOVED)를, `git diff`로 수정(MODIFIED)을 각각 따로 감지해서 배포 자체를 막는다(아래 검증 이력 참고).

### baseline이란

Flyway를 **이미 데이터가 있는 DB**(빈 DB가 아니라)에 처음 붙일 때 문제가 생긴다 — 장부가 텅 비어있으니 Flyway는 "V1부터 하나도 안 적용됨"이라고 오해하고 처음부터 다 실행하려 든다. `baseline`은 "여기까지는 이미 적용된 걸로 치자"고 사람이 선언하는 지점이다.

이 프로젝트(#133, 2026-07-23 도입)는:
```yaml
baseline-on-migrate: true
baseline-version: 1   # V1(baseline.sql)까지는 이미 적용된 걸로 간주
```
`V1__baseline.sql`엔 그 시점 엔티티 매핑이 실제로 만드는 전체 스키마를 캡처해뒀다(`ddl-auto=create`로 빈 DB에 띄워 Hibernate가 낸 SQL을 그대로 옮김, 실제 stage DB 사본으로 `ddl-auto=validate` 통과까지 확인). 이미 떠 있던 stage·prod는 이 파일을 **실행하지 않고** 장부에만 "V1까지 적용됨"으로 표시된다 — 즉 baseline은 "이 시점 이후로만 Flyway가 책임진다"는 경계선이고, **그 경계선 안쪽(과거)의 실제 스키마가 baseline SQL과 정말 같은지는 Flyway가 검증해주지 않는다**(사람이 `ddl-auto=validate`로 미리 확인해야 하는 몫). 실제로 baseline 도입 이전(2026-07-17) 백업으로 복원을 시도했을 때 이 가정이 깨져서 기동 실패가 난 적이 있다(`db-migration.md` 참고) — 백업에서 복원할 땐 항상 baseline 도입일 이후 스냅샷만 써야 하는 이유다.

### `migrate()` vs `clean()` vs `repair()` — 셋은 완전히 다른 일을 한다

| 명령 | 실제로 하는 일 | 데이터 영향 |
|---|---|---|
| `migrate()` | 아직 안 적용된 **새** 마이그레이션 파일을 순서대로 실행 | 새로 추가된 내용만큼 스키마·데이터가 바뀜(정상 동작) |
| `clean()` | DB의 **모든 테이블을 drop** | 전체 삭제 |
| `repair()` | 장부(`flyway_schema_history`)만 정리 — 없어진 파일의 기록 삭제, 바뀐 파일의 체크섬 갱신 | **없음** — 테이블은 전혀 안 건드림 |

`repair()`는 "복구"라는 이름과 달리 실제 스키마·데이터를 되돌리거나 다시 만드는 능력이 전혀 없다 — 오직 "Flyway 자신이 알고 있는 상태(장부)"를 "지금 실제 파일 목록"에 맞춰 조정할 뿐이다. 파일이 지워졌으면 그 버전의 기록을 지우고(그 마이그레이션이 예전에 실행해서 만든 실제 스키마 변경은 그대로 남는다 — Flyway가 로그로 직접 경고하는 부분이다), 파일 내용이 바뀌었으면 체크섬만 새 내용 기준으로 갱신한다. 진짜로 스키마·데이터를 고쳐야 하면 그건 새 마이그레이션 파일을 만들어 `migrate()`로 **앞으로** 적용하는 것만이 정답이다 — Flyway는 원래 "뒤로 돌아가 지우는" 도구가 아니라 "앞으로만 쌓아가는" 도구이기 때문이다.

**왜 처음엔 `clean()`을 자동 안전장치로 썼었나, 그리고 왜 그게 애초에 이 프로젝트의 Flyway 도입 취지와 어긋났나**: 이 프로젝트가 Flyway를 도입한 이유(#133, `pm/docs/learnings.md`)는 "스키마 변경을 암묵적으로 처리하다가 실패를 조용히 삼키는" 사고를 막기 위해서였다 — DDL은 명시적인 SQL 파일로, 순서대로, 기록을 남기며 하자는 게 취지. 그런데 검증 오류가 나면 자동으로 `clean()`(전체 삭제 후 재구성)을 실행하는 안전장치는 정확히 그 반대 방향이다 — "안 맞으면 그냥 다 지우고 새로 만들자"는, Flyway를 도입하기 전 `ddl-auto: update`가 하던 암묵적 처리와 본질적으로 같은 사고방식이고, 그 결과가 2026-08-01 stage DB 전체삭제 사고였다(아래 검증 이력 참고). `repair()`로 바꾼 건 안전장치의 "실행 능력"을 없앤 것이고, 대신 판단·차단은 `migration-check` job(사람이 볼 수 있게 배포를 막음)이 맡는다 — **실행 기록은 Flyway, 판단은 CD/사람**으로 역할을 분리한 것이 이번 수정의 핵심이다.
