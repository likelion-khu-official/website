...
**같은 판별이 `UPDATE`/`DELETE` DML도 본다(2026-07-31 실측 보강).** 기존 행을 바꾸거나 지우는 마이그레이션(백필·정정 스크립트 등)은 컬럼 삭제·이름변경이 아니라도 자동 롤백 전에 사람이 보게 한다 — 롤백은 앱 이미지만 되돌릴 뿐 이미 실행된 `UPDATE`/`DELETE`는 그대로 남기 때문이다. `INSERT`(행 추가)는 계속 안전 취급이니 시드 데이터 삽입 같은 건 예전처럼 자유롭게 써도 된다.
