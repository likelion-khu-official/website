...
description: >-
  ...
  "GUI 열어줘/터널 열어줘/DB 화면 켜줘" 등을 물을 때. 인프라 오너(장찬욱)가 팀원에게 실제
  공개키를 받아 `dbclient`/`dbtunnel` 계정에 등록해야 할 때도 이 스킬로 처리한다.
---
...
**"이거 조회/등록/수정해줘" (등록된 팀원이 실제 쿼리 실행을 요청할 때)**
→ 팀원은 SSH 명령을 직접 안 치고 본인 Claude Code(이 레포를 체크아웃한 환경이면 이 스킬을 자동으로 인식함)한테 요청하는 게 기본 사용법이다. 요청받으면:
1. `db-access.md`의 "현재 등록 상태"로 요청자가 실제 등록됐는지 확인(모르면 직접 SSH 시도해보면 안 됐을 때 인증 실패로 바로 드러남 — 등록 안 됐으면 장찬욱에게 등록부터 요청하라고 안내).
2. `ssh dbclient@168.138.202.82 stage`(또는 `prod`)에 SQL을 파이프로 넣어 실행한다(예: `printf 'SELECT ...;\n' | ssh dbclient@168.138.202.82 stage`). 요청자가 스키마 변경(ALTER/CREATE/DROP)을 요청하면 실행 전에 먼저 "이건 기술적으로 막혀 있고 마이그레이션 파일 + PR로 해야 한다"고 안내하고 실행하지 않는다(래퍼가 어차피 막지만, 시도조차 안내 없이 하지 않는다).
3. **prod에 대한 `INSERT`/`UPDATE`/`DELETE`는 실행 전 요청자에게 한 번 더 확인**(비권장 영역이라 되돌릴 수 있는 방법이 백업뿐이므로) — stage 조작이나 조회(`SELECT`, stage/prod 무관)는 바로 실행해도 된다.
4. 결과를 그대로 요청자에게 전달한다 — 가공하거나 요약만 하지 말고, 실제 행/값을 보여줘야 확인이 된다.
...
┌─────────────────────────┬──────────────────────────┐
│ pane 1: 조회 (브라우저)    │ pane 2: 조작 (CLI, 기존)     │
│ ssh -N -L 8090:127.0.0.1 │ ssh dbclient@host stage    │
│   :8090 dbtunnel@host    │                             │
│ → localhost:8090 오픈    │                             │
└────────────┬─────────────┴──────────────┬──────────────┘
             │                            │
...
│  ├ permitopen=127.0.0.1:           → guard.sh            │
│  │   8090/8091                   └ DML만 허용, DDL 차단   │
│  └ 그 외 포워딩·셸 전부 불가                                │
│         │                                                │
│         ▼                                                │
│  sqlite-web-stage :8090 / sqlite-web-prod :8091           │
│  (127.0.0.1 바인딩, -r read-only + :ro 마운트 이중 방어)     │
│         │                                                │
│         ▼                                                │
│    stage.db / prod.db  ← dbclient가 조작하는 파일과 동일     │
└──────────────────────────────────────────────────────┘
공인 노출 변화 없음(22/80/443만 오픈, 8090/8091은 127.0.0.1 바인딩)
```
