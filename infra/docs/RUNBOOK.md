...
└───────────────────────────┬──────────────────────────────┘
                            ...
                            ▼
┌──────────────────────────────────────────────────────────┐
│  OCI Monitoring — Alarm Definitions × 7                    │
...
**각 알람은 경고(왜 울렸나) → 영향(지금 뭐가 실제로 안 되고 있나) → 원인 후보(자주 나오는 원인) → 복구(뭘 하면 되나) 순서로 정리돼 있다**(2026-07-27 김우진 리뷰로 추가 — 알람을 받고도 "이게 지금 진짜 급한 건가, 뭐부터 의심해야 하나"를 매번 처음부터 판단해야 하는 게 문제였다). 제목 옆 **대응 긴급도**는 OCI가 매긴 심각도(디스크·메모리·백업부재·git드리프트는 `CRITICAL`, 이메일실패는 `WARNING` — 이유는 `observability.md` 참고)와는 별개로, 지금 실제로 서비스에 영향이 가고 있는지를 기준으로 우리가 따로 판단해 붙인 것이다 — **예방적 경고**는 아직 서비스는 멀쩡하고 미리 손볼 여유가 있는 알람, **즉시 대응**은 이미 사용자 쪽에 영향이 가고 있는 알람이다.
...
**영향**: 이 문서에 나오는 알람 중 유일하게 "이미 사용자가 실제로 접속 못 하고 있는" 상태다 — 1-1~1-4, 1-6은 전부 방치하면 언젠가 문제가 되는 조기 경보지만, 이건 지금 이 순간 영향이 발생 중이라는 점에서 다르다. 그래서 여러 알람 중 가장 먼저 확인해야 한다.
...
**수동 롤백** — 배포 자동화(CD)는 이번 배포에 포함된 마이그레이션이 컬럼 추가만 하는 "안전한" 종류일 때만 자동으로 롤백한다(2026-07-31, #320 — 이유는 `infra/CLAUDE.md` "수동 배포·롤백" 절 참고). 그 자동 롤백조차 실패했거나, 컬럼 삭제·이름변경형 마이그레이션이라 애초에 자동 롤백을 건너뛰고 실패로 끝난 경우엔 사람이 판단해야 한다:

1. **먼저 fix-forward를 검토한다** — 실패 원인(대부분 마이그레이션과 무관한 설정·코드 문제)을 고쳐서 같은 버전으로 재배포. 앱·DB가 계속 같은 버전으로 맞물리니 가장 빠르고 안전하다.
2. 그래도 이전 이미지로 되돌려야 하면, 아래 명령에 이전에 정상 동작했던 태그를 넣어 실행한다. **단, 컬럼 삭제·이름변경형 마이그레이션 뒤라면 구버전 앱이 바뀐 스키마와 안 맞아 또 실패할 수 있다는 걸 인지하고 진행할 것.**
3. DB까지 되돌려야 한다고 판단되면, 삭제/변경형 마이그레이션 배포 시 CD가 자동으로 떠둔 사전 백업(`db-access.md` "백업 전략")과, 아래 "DB 복원" 절차 ④에서 자동으로 남는 `.before-restore.*` 스냅샷을 재료로 데이터 손실 없이 판단한다.
