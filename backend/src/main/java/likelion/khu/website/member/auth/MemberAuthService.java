...
public class MemberAuthService {
    ...
    // 본인이 직접 바꾸는 경우(첫 로그인 강제 변경 포함). access 토큰은 발급 시점 클레임을 그대로 담고 있어
    // DB의 mustChangePassword를 내려도 이미 나간 토큰엔 반영이 안 된다 — 그래서 기존 세션(다른 기기 포함)은
    // 전부 끊고, 방금 요청한 세션에는 mcp=false가 반영된 새 토큰 쌍을 바로 발급해 다시 로그인할 필요가 없게 한다.
    @Transactional
    public LoginResult changePassword(Long memberId, String newRawPassword) {
