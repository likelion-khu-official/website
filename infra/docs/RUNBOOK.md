...
**경고**: 최근 15분 안에 `email_log`에 `FAILURE`로 기록된 행이 2건을 넘었고, 그 상태가 20분 넘게 지속됐다는 뜻이다(연속 2번의 15분 집계가 모두 임계치를 넘어야 파이어 — 설계 근거는 `observability.md` "알람 튜닝 사고 모델" 적용 예시 참고). 개별 실패 1~2건은 정상 범위(주소 오탈자 등)라 이 알람 자체가 "시스템적으로 계속 실패 중"이라는 신호다.
