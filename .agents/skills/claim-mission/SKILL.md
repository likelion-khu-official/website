...
1. **R — research** *(자율)* → `kb/research.md`. 미션의 `Deliverable`·`Done`·`Notes`를 기준으로 코드·계약(`shared/`)·주변을 읽어 "무엇을 해야 하는가"를 파악. **미션의 `Target` Epic 이슈(이 미션이 sub-issue로 달린 상위 Epic)와 그 Epic이 가리키는 위키 스펙을 먼저 읽어라** — 이게 이 미션이 전진시키는 기능 슬라이스고, 무엇·왜의 정본이다. 사람에겐 "지금 이 미션이 뭘 요구하는지 파악 중이에요"라고만.
...
4. **현황은 자동 롤업 — 손으로 안 고친다.** 미션(Story)을 닫으면(위 close-mission) 이 미션이 sub-issue로 달린 상위 Epic의 진행률이 GitHub에서 자동으로 갱신된다. 예전의 손유지 상태 트리(`pm/features`·README)는 폐기됐다 — 상태 색을 칠할 일이 없다. (그 Epic의 하위 미션이 이번으로 다 닫혔으면, PM이 project-board/tidy-missions로 Epic을 닫는다 — 팀원은 여기까지 안 해도 된다.)
