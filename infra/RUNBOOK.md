...
다음 기수로 인수인계하는 방법을 담은 `pm/docs/handoff.md`에서 김우진이 이렇게 물은 적이 있다 — "가끔 알림이 오는데, 받으면 뭘 해야 하는지 정해져 있어?" 그 문서는 "정해져 있지 않다, 이게 실제 공백이다"라고 답하고, 이 러너북을 만들어야 할 다음 할 일로 남겼다.

처음엔 그 공백(알람이 왔을 때 뭘 하는지)만 메우면 될 줄 알았는데, 그것만으론 부족했다. **알람이 없을 때는 뭘 해야 하는지, 애초에 이 일을 왜 하는지**까지 담아야 한다 — 그게 빠지면 다음 담당자는 알람이 울릴 때만 움직이는 사람이 되어 버리고, 그건 이 역할이 원래 해야 하는 일이 아니다.
...
**이 역할의 진짜 목표는 "인프라를 유지하는 것"이 아니라 "팀 전체가 인프라를 신경 쓰지 않고 자기 일에 집중할 수 있게 만드는 것"이다.** 서버가 안 죽는 것, 알람에 잘 대응하는 것은 그 목표를 이루기 위한 수단이지 목표 자체가 아니다. 그래서 서버가 멀쩡히 떠 있어도, 백엔드·프론트가 배포를 무서워하거나 DB 접근을 못 해 기다리거나 문서가 실제와 달라 잘못 판단하게 만든다면 이 역할은 실패한 것이다.

**이 역할을 물려받는 사람에게**: 이 문서에 적힌 절차를 그대로 따라 하기만 해서는 안 된다. 팀 규모, 트래픽, 무료 티어 정책, 팀원 각자의 필요는 계속 바뀌는데 문서만 고정돼 있으면, 그 문서 자체가 다음 병목이 된다. **여기 적힌 걸 그대로 지키는 게 아니라, 상황에 안 맞으면 스스로 판단해서 고치고 이 문서도 같이 갱신하는 것**이 맡아야 할 일이다. 이 문서를 "정답집"이 아니라 "지금까지 이런 식으로 판단해왔다"는 견본으로 봐야 하는 이유가 여기 있다.

### 실제 일하는 방식 — 이 문서를 쓴 과정 자체가 예시다

이 러너북과 관련 문서들은 한 번에 완성되지 않았다. 아래 순서를 반복하면서 만들어졌고, 다음 담당자도 이 순서를 그대로 가져가면 된다:
