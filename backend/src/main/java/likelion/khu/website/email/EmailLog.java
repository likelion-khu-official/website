...
public class EmailLog {
    ...
    // FAILURE 행에만 값이 채워짐(SUCCESS는 null) — #113 실패 임계치 알람이 유저 원인(주소 형식
    // 오류)은 세지 않고 우리 쪽 원인만 세도록 email_log에 남겨두는 분류. FailureCause 참고.
    @Enumerated(EnumType.STRING)
    ...
    private EmailLog(String recipient, EmailType emailType, String subject, EmailStatus status,
                      String errorMessage, FailureCause failureCause, String messageId) {
    ...
    public static EmailLog success(String recipient, EmailType emailType, String subject, String messageId) {
        return new EmailLog(recipient, emailType, subject, EmailStatus.SUCCESS, null, null, messageId);
    }

    public static EmailLog failure(String recipient, EmailType emailType, String subject,
                                    String errorMessage, FailureCause failureCause, String messageId) {
        return new EmailLog(recipient, emailType, subject, EmailStatus.FAILURE, errorMessage, failureCause, messageId);
    }
}
