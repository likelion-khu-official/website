...
## Epic과 한 몸이다 (불변식 — 제일 먼저 읽어라)

미션은 공중에 뜬 일이 아니다. **한 Epic(사용자 향 기능 슬라이스, 예: "방문자가 프로젝트 목록을 볼 수 있다")을 전진시키는 분야별 실작업(Story)**이다. 이 커플링이 미션 체계의 척추다:

- **위키 = 스펙(무엇이어야 하나) · Epic 이슈 = 추적 단위(뭐가 됐나) · 미션(Story) = Epic을 움직이는 분야별 실작업.** Epic의 진행률은 하위 미션 상태로 GitHub이 **자동 롤업**한다 — 손으로 상태를 유지하지 않는다.
- **미션 백로그 = 아직 미션이 안 달린 Epic**(또는 위키에 스펙만 있고 Epic이 없는 기능). "일 줘 / 미션 만들어"를 받으면 **먼저 열린 Epic 중 다음에 밀 것을 고른다**(`gh issue list --repo likelion-khu-official/website --label epic --state open`). 없는 일을 지어내지 말고, 이미 정의된 기능에서 다음을 고른다.
- **미션의 무엇·왜는 Epic·위키에서 나온다.** 대상 Epic의 `## 목표`·`## 완료기준`과 그 Epic이 링크한 위키 스펙이 *무엇·왜*다. 미션은 이걸 한 분야의 실행 단위로 옮길 뿐 새로 발명하지 않는다.
- **발주하면 그 미션을 Epic에 sub-issue로 건다(필수).** Epic이 아직 없으면(위키에만 스펙이 있는 새 기능) **먼저 그 기능의 Epic을 만들고**(아래 "Epic 지연 생성"), 그 밑에 미션을 건다. 진행률·현황은 이 링크로 자동 굴러가니, 예전처럼 상태 색·`진행 상태`를 손으로 갱신할 일이 없다(그 손유지가 2026-07-15 트리가 며칠간 거짓말한 사고의 원인이었다 — 그래서 손유지 상태 트리 `pm/features`를 폐기했다).

### Epic 지연 생성 (미션 발주 시, Epic이 없으면)
이 기능 슬라이스의 Epic 이슈가 아직 없으면 발주 직전에 만든다 — 있으면 재사용(멱등):
1. 열린 Epic에서 이 기능이 있는지 찾는다: `gh issue list --repo likelion-khu-official/website --label epic --state open`.
2. 없으면 Epic 이슈를 만든다 — 제목 `[Epic] <사용자가 ~할 수 있다>`, 라벨 `epic` + `area:<테마>`, **assignee·`roadmap` 라벨 없음**(그래야 claim-mission이 안 집는다). 본문 4부: `## 목표`(사용자 가치) · `## 완료기준`(위키·GWT 기반 안정 계약, 진행상태 안 섞고 검증 ID는 블록 끝에 한 줄로) · `## 스펙`(위키 링크만, 인라인 금지) · `## 하위 Story`.
3. 만든 뒤 미션을 그 Epic 밑에 건다: `pm/scripts/link-subissue.sh <Epic#> <미션#>`.
...
- **`Target`** — 이 미션이 전진시키는 **Epic 이슈**(기능 슬라이스). Epic 이슈 링크(예: `#200`)로 건다 — 미션은 반드시 한 Epic의 sub-issue가 된다(이게 미션↔제품 현황을 잇는 고리다). Epic이 아직 없으면 위 "Epic 지연 생성"으로 먼저 만든다. `Deliverable`·`Why`·`Done`은 이 Epic의 `## 완료기준`과 그것이 링크한 위키 스펙에서 파생한다.
...
## 품질 루브릭 (저장/생성 전 자가검증)
- **대상 Epic** — `Target`이 실재 Epic 이슈를 가리키나. 없으면 미션이 Epic에 안 걸린다(→ 위 "Epic 지연 생성"으로 먼저 만든다).
...
5. **검증(누락 방지):** 던진 직후 카드가 실제로 붙었는지 확인. `0`이면 4를 안 한 것 — 다시 올린다.
   ...
   ```
6. **상위 Epic에 sub-issue로 건다(필수).** 이 미션의 `Target` Epic 밑에 방금 만든 미션을 연결한다 — Epic이 없었으면 위 "Epic 지연 생성"으로 먼저 만들고 건다.
   ```
   pm/scripts/link-subissue.sh <Epic#> <n>
   ```
   빼먹으면 미션이 Epic/롤업 밖에 떠서 현황에 안 잡힌다.
...
**현황은 자동으로 갱신된다 — 손으로 트리를 고치지 않는다.** 미션(Story)을 닫으면 상위 Epic의 진행률이 자동 롤업되고, 그 Epic의 하위 미션이 다 닫히면 Epic도 닫는다(→ project-board / tidy-missions 스킬). 예전의 손유지 상태 트리(`pm/features`)는 폐기됐다 — 발주·닫기 때 상태 색을 칠할 일이 없다.
