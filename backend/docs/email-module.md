...
| `EmailLogRecorder.java` | `email_log` 기록 전용 — `@Transactional(REQUIRES_NEW)`로 호출자의 트랜잭션과 무관하게 항상 별도 트랜잭션에 즉시 커밋(#85 리뷰, 신선우 — 아래 트랜잭션 경계 절 참고) |
| `EmailService.java` | 수신자 주소 검증(`InternetAddress.validate()`) → Thymeleaf 렌더링 → `JavaMailSender` 발송 → 성공/실패 무조건 `EmailLogRecorder`로 `email_log` 기록. stage 프로파일이면 제목에 `[stage] ` 접두어 |
...
**수정**: `email_log` 저장을 `EmailLogRecorder`라는 별도 빈으로 분리하고 `@Transactional(propagation = REQUIRES_NEW)`를 붙였다. 같은 클래스 안에서 호출하면 self-invocation이라 프록시를 안 타서 `@Transactional`이 무시되므로, 반드시 `EmailService` 밖의 별도 빈이어야 한다(`EmailLogRecorder` 클래스 주석 참고). 성공 로그도 같은 문제(발송 성공 후 트랜잭션이 다른 이유로 롤백되면 성공 기록도 사라짐)에 노출돼 있어 대칭적으로 함께 고쳤다.

**재현 테스트**: `TransactionalEmailInviter`(테스트 전용, `@Transactional` 메서드로 `#74` 호출 형태를 흉내)를 통해 —
- `EmailServiceFailureIntegrationTest#sendInviteEmail_CalledInsideTransactionThatRollsBack_FailureLogSurvivesRollback` — SMTP 실패로 바깥 트랜잭션이 롤백돼도 실패 로그가 남는지
- `EmailServiceIntegrationTest#sendInviteEmail_SucceedsButOuterTransactionLaterRollsBackForUnrelatedReason_SuccessLogSurvivesRollback` — 발송은 성공했지만 이후 다른 이유로 바깥 트랜잭션이 롤백돼도 성공 로그가 남는지

두 테스트 모두 로컬에서 Docker 없이는 못 돌려봤음(Testcontainers 필요) — CI(`ubuntu-latest`, Docker 기본 설치)에서 실행 확인 필요.
...
- [x] 11. `EmailLogRecorder.java` — 트랜잭션 경계 리뷰(#85, 신선우) 반영. `email_log` 저장을 `REQUIRES_NEW`로 분리해 호출자 트랜잭션 롤백과 무관하게 만듦 — 위 "트랜잭션 경계" 절 참고. 재현 테스트 2개 추가(`EmailServiceFailureIntegrationTest`·`EmailServiceIntegrationTest`), 로컬 Docker 미설치로 CI 확인 필요
