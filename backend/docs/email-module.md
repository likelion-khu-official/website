...
| `EmailService.java` | 활성 트랜잭션 안에서 호출됐는지 먼저 가드 → 수신자 주소 검증(`InternetAddress.validate()`) → Thymeleaf 렌더링 → `JavaMailSender` 발송 → 성공/실패 무조건 `email_log` 기록. stage 프로파일이면 제목에 `[stage] ` 접두어. 가드가 왜 필요한지는 아래 "트랜잭션 경계" 절 참고 |
...
| 11 | 활성 트랜잭션 안에서 호출 시 즉시 거부(가드) | `@Transactional` 메서드 안에서 `sendInviteEmail(...)` 호출 | `IllegalStateException` 즉시 던짐(메시지에 "트랜잭션" 포함), `mailSender.send()` 시도 안 함, `email_log`에 아무 행도 안 남음 | 단위: `EmailServiceTest#send_CalledWithActiveTransaction_ThrowsImmediatelyWithoutAttemptingSendOrLog`(`TransactionSynchronizationManager`를 직접 조작해 DB·Spring 컨텍스트 없이 재현) / 통합: `EmailServiceTransactionGuardIntegrationTest#sendInviteEmail_CalledInsideRealTransactionalProxy_...`(진짜 `@Transactional` 프록시로 재현) |
...
### 최종 수정 — 조용히 사라지는 대신 즉시 크게 실패

"바깥 트랜잭션과 무관하게 독립 커밋"이 이 DB에서 불가능하므로, 대신 `EmailService.send()`가 **활성 트랜잭션 안에서 호출되면 아무것도 시도하지 않고 그 자리에서 즉시 `IllegalStateException`을 던지도록** 가드를 넣었다(`TransactionSynchronizationManager.isActualTransactionActive()`). email_log가 조용히 유실되는 대신, `#74`가 이 메서드를 트랜잭션 안에서 부르는 순간 개발 중에 바로 시끄럽게 터져서 알아채게 만드는 방향 — 이 커넥션 풀 제약 안에서 낼 수 있는 최선의 방어다. `#74` 구현 시엔 토큰 저장을 자기 트랜잭션으로 먼저 커밋하고, `EmailService` 호출은 트랜잭션 밖에서 해야 한다.

**재현 테스트**: `TransactionalEmailInviter`(테스트 전용, `@Transactional` 메서드로 `#74` 호출 형태를 흉내)를 통해 위 표 11번 항목 검증. 통합 버전(`EmailServiceTransactionGuardIntegrationTest`)은 Mailpit 컨테이너가 필요 없다 — 가드가 SMTP 시도보다 먼저 막기 때문.
...
- [x] 11. 트랜잭션 경계 리뷰(#85, 신선우) 반영. 1차 시도(`EmailLogRecorder` + `REQUIRES_NEW`)는 CI에서 실패 — SQLite 커넥션 풀(=1)과 구조적으로 안 맞음을 실측. 대신 `EmailService.send()`에 활성 트랜잭션 감지 가드를 넣어 즉시 실패하게 수정 — 위 "트랜잭션 경계" 절 참고. `TransactionalEmailInviter` + `EmailServiceTransactionGuardIntegrationTest`로 재현 테스트 완료, 로컬 단위테스트 10/10 통과 확인(통합은 로컬 Docker 미설치로 CI 확인 필요)
