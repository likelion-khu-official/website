...
| OCI Notifications(ONS) 구독 — 토픽 `likelion-ops-alerts` | 동아리 공용 메일 + 김우진(PM) 개인 메일이 구독 중, 장찬욱 개인 메일은 **애초에 구독 안 돼 있음** | 디스크 80%↑·메모리 85%↑·백업 26h 부재·git 드리프트 — 이 4개 알람의 **유일한 도달 경로**(전부 이메일로만 옴) | 콘솔 `Notifications → Topics → likelion-ops-alerts`에서 후임자 메일 구독 추가(PENDING으로 뜨니 본인이 메일함에서 확인 클릭까지 해야 활성화) — 불필요해진 사람은 구독 해지. |
...
3. ONS 구독 추가(+ 본인 확인 클릭) — ①②와 순서 무관, 오히려 일찍 해둘수록 인수인계 기간 동안 후임자도 실제 알람을 같이 받아보며 감을 잡을 수 있다.
4. GitHub Secrets 갱신(SSH 키를 교체했다면 `OCI_SSH_KEY`는 필수).
5. UptimeRobot·DNS 등록업체는 이미 공용 계정이라 로그인 정보만 전달.
6. 위 전부 확인된 뒤에만 장찬욱 개인 계정·키를 제거한다.
